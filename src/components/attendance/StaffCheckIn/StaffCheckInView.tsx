  ...
  return (
    <div className="p-4 md:p-6 md:max-w-lg md:mx-auto space-y-5">
      ...
      <div className="space-y-3 pt-1">
        <div>
          <h1 className="text-2xl font-semibold">{t("Hey")} {firstName} 👋</h1>
      ...
      {step === "locating" && (
        <Card className="py-0">
          <CardContent className="flex flex-col items-center gap-3 py-10">
            ...
            <p className="text-sm text-muted-foreground">{t("Verifying your location…")}</p>
          </CardContent>
        </Card>
      )}
      ...
      {step === "location_error" && (
        <Card className="py-0">
          <CardContent className="flex flex-col items-center gap-4 py-8 text-center">
            ...
            <Button variant="outline" onClick={resetFlow}>{t("Try Again")}</Button>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="py-4">
          <CardHeader className="pb-3">
            <CardTitle className="text-base">
              {t("Confirm")} {mode === "checkin" ? t("Check In") : t("Check Out")}
            </CardTitle>
          </CardHeader>
            ...
            <div className="space-y-1.5 text-sm">
              {locationPending ? (
                ...
                <p className="flex items-center gap-2">
                  ...
                  {t("Location verified")} ({distanceM} {t("m from branch")})
                </p>
              )}
              <p className="flex items-center gap-2">
          ...
          {!checkedIn && precheckStep === "error" && (
            <Card className="py-0">
              <CardContent className="flex flex-col items-center gap-4 py-8 text-center">
                ...
                <Button variant="outline" onClick={retryPrecheck}>{t("Try Again")}</Button>
              </CardContent>
            </Card>
          )}
              ...
              <CardContent className="flex flex-col items-center gap-5 py-8 text-center">
                ...
                <div>
                  <p className="font-semibold">
                    {withinWindow
                      ? activeShift
                        ? `${t("Ready for")} ${activeShift.name}?`
                        : t("Ready to start your shift?")
                      : t("No open check-in window")}
                  </p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {withinWindow
                      ? t("Your location and photo will be verified")
                      : shiftStartTime
                      ? `${t("Next window opens at")} ${formatShiftTime(shiftStartTime)} (±${windowMins} ${t("min")})`
                      : t("Check the shifts above for today's schedule")}
                  </p>
                </div>
                <Button
              ...
              <CardContent className="flex flex-col items-center gap-5 py-8 text-center">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-sm text-muted-foreground">
                    {t("Checked in at")} {format(parseISO(todayLog.check_in_at!), "h:mm a")}
                    {todayLog.is_late && (
                      <span className="ml-1.5 text-amber-600 dark:text-amber-400">
                        · {t("Late by")} {todayLog.late_minutes} {t("min")}
                      </span>
                    )}
                  </p>
                </div>
            ...
            <Card className="py-4">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="rounded-lg bg-muted/50 p-3">
                    <p className="text-xs text-muted-foreground">{t("Check Out")}</p>
                    <p className="mt-0.5 text-base font-semibold tabular-nums">
                      {format(parseISO(todayLog.check_out_at!), "h:mm a")}
                    </p>
                  </div>
                  <div className="rounded-lg bg-muted/50 p-3">
                    <p className="text-xs text-muted-foreground">{t("Hours Worked")}</p>
                    <p className="mt-0.5 text-base font-semibold tabular-nums">
                      {todayLog.total_hours?.toFixed(1) ?? "—"} {t("hrs")}
                    </p>
                  </div>
                  <div className="rounded-lg bg-muted/50 p-3">
                    <p className="text-xs text-muted-foreground">{t("Day Value")}</p>
                    <p className="mt-0.5 text-base font-semibold tabular-nums">
      ...
      {history.length > 0 && (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-muted-foreground">{t("Last 14 days")}</h2>
          </div>
          <div className="space-y-2">
            {history.map((log) => (
              <div
